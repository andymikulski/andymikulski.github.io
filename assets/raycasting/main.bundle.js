...
class RenderingPipeline {
    constructor() {
        ...
        this.viewportCords = [0, 0];
    }
    createCanvas(width, height) {
        const output = new CanvasRenderer_1.default(height, width);
        this.viewport = new QuadTree_1.Rect(this.viewportCords[0], this.viewportCords[1], width, height);
...
class QuadTree extends emitter_1.default {
    ...
    static getDirection(x, y, region) {
...
class DDATestApp {
    constructor(cellSize = 10, gridSize = 15) {
        ...
        this.pipeline = new RenderingPipeline_1.default();
        // The pipeline needs to generate a canvas to display things on to the user.
        const totalCellCount = this.cellSize * this.gridSize;
        this.pipeline.createCanvas(window.innerWidth, window.innerHeight);
        this.dotMan = new dot_1.DotManager(totalCellCount, this.cellSize);
        ...
        const getFPS = () => {
            ...
            lastLoop = thisLoop;
            this.fps = fps;
            requestAnimationFrame(getFPS.bind(this));
        };
        getFPS();
        const tickMouse = () => {
        ...
        window.addEventListener('mousemove', (evt) => {
            mouseX = Math.floor(evt.clientX / this.cellSize) - 1;
            mouseY = Math.floor(evt.clientY / this.cellSize) - 1;
            this.render();
        });
        const playerSpeed = 1;
        window.addEventListener('keydown', (evt) => {
            ...
            switch (evt.which) {
                ...
                case 87: // up
                case 38:// up
                    nextX = this.playerPos[0];
                    nextY = this.playerPos[1] - playerSpeed;
                    if (this.isOnMap(nextX, nextY) && !this.wallPoints[`${Math.round(nextX)},${Math.round(nextY)}`]) {
                        this.playerPos = [nextX, nextY];
                    }
                    break;
                case 83: // up
                case 40://down
                ...
                case 68:
                case 39:// right
                    nextX = this.playerPos[0] + playerSpeed;
                    nextY = this.playerPos[1];
                    if (this.isOnMap(nextX, nextY) && !this.wallPoints[`${Math.round(nextX)},${Math.round(nextY)}`]) {
                        this.playerPos = [nextX, nextY];
                    }
                    break;
        ...
        for (var i = 0; i < this.gridSize % 3; i += 1) {
            ...
            if (mode === 0) {
                ...
                const y1 = Math.floor(Math.random() * (this.gridSize));
                const y2 = Math.floor(Math.random() * (this.gridSize));
                buildWall([x, y1], [x, y2]);
            }
            else if (mode === 1) {
                const y = Math.floor(Math.random() * (this.gridSize));
                const x1 = Math.floor(Math.random() * (this.gridSize));
                const x2 = Math.floor(Math.random() * (this.gridSize));
                buildWall([x1, y], [x2, y]);
            }
            else if (mode === 2) {
                const y = Math.floor(Math.random() * (this.gridSize));
                const x1 = Math.floor(Math.random() * (this.gridSize));
                const x2 = Math.floor(Math.random() * (this.gridSize));
                const midPoint = Math.round(x2 - x1);
                buildWall([x1, y], [midPoint, y]);
                buildWall([midPoint, y], [x2, y]);
            }
            else if (mode === 3) {
                const x = Math.floor(Math.random() * (this.gridSize));
                const y1 = Math.floor(Math.random() * (this.gridSize));
                const y2 = Math.floor(Math.random() * (this.gridSize));
                const midPoint = Math.round(y2 - y1);
                buildWall([x, y1], [x, midPoint]);
                buildWall([midPoint, y1], [x, y2]);
            }
        }
        this.render();
    ...
    render() {
        this.dotMan.clear();
        // this.pipeline.addRenderer(new Grid(this.cellSize, this.gridSize));
        // buildWall([10, 5], [10, 8]);
        // // buildWall([0, 15], [30, 5]);
        // // buildWall([0, 8], [8, 0]);
        // buildWall([7, 7], [7, 9]);
        ...
        // buildWall([4, 5], [8, 5]);
        // buildWall([9, 7], [7, 7]);
        Object.values(this.knownWalls).forEach(pt => {
            this.dotMan.add(this.cellSize, pt, 'rgba(255,100,100,0.33)');
        });
        let startAngle = this.turnAngle;
        const angleRange = this.fov;
        this.visited = {};
    ...
    getLinePoints(pt1, pt2) {
        ...
        for (let i = 0; i <= steps; i += 1) {
            const pt = [(currentX), (currentY)];
            // const flooredPt = [Math.floor(currentX), Math.floor(currentY)];
            // const ceiledPt = [Math.ceil(currentX), Math.ceil(currentY)];
            points.push(pt);
            // points.push(flooredPt);
            // points.push(ceiledPt);
            // this.dotMan.add(this.cellSize, pt, 'rgba(100, 10, 10, 1)');
            // this.dotMan.add(this.cellSize, flooredPt, `rgba(100, 10, 10, ${1.25 - (currentY) % 1}) `);
            // this.dotMan.add(this.cellSize, ceiledPt, `rgba(100, 10, 10, ${(currentY) % 1}) `);
            // this.dotMan.add(this.cellSize, [(currentX), (currentY)], 'rgba(255, 0, 0, 0.2)');
            currentX += xInc;
            currentY += yInc;
        }
        return points;
    ...
    drawLine(pt1, pt2) {
        const points = this.getLinePoints(pt1, pt2);
        points.forEach(pt => {
            this.dotMan.add(this.cellSize, pt, 'rgba(100, 10, 10, 1)');
        });
    }
